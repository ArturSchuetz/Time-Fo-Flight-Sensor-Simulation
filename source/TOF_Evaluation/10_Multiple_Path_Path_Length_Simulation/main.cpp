			...
			if (mean_depthMat.cols > 0 && mean_depthMat.rows > 0 && reference_depthMat.cols > 0 && reference_depthMat.rows > 0)
			{
				...
				if (differenceMat.rows > 0 && differenceMat.cols > 0)
				{
					...
					for (unsigned int row = 0; row < differenceMat.rows; row++)
					{
						...
						{
							float value = differenceMat.at<float>(row, col) * 255.0f / 100.0f;
							if (value < 0)
								value = -value;
					...
					// ========================================================================
					cv::Mat cm_img0;
					cv::imwrite(recordingsFolderPath + "\\" + recordedFiles[dirIndex].folderName + "_shadow_error_diffmap.png", diff_mat);

					applyColorMap(diff_mat, cm_img0, cv::COLORMAP_HOT);
					cv::imwrite(recordingsFolderPath + "\\" + recordedFiles[dirIndex].folderName + "_shadow_error_heatmap.png", cm_img0);

					applyColorMap(diff_mat, cm_img0, cv::COLORMAP_JET);
					cv::imwrite(recordingsFolderPath + "\\" + recordedFiles[dirIndex].folderName + "_shadow_error_jetmap.png", cm_img0);
				}
...
{
	g_renderer.Start();
	runAnalysis("/data/Kinect_v2_Calibration.xml", "F:\\Kamera_Evaluation\\\Simulation\\Recordings_Path_Length");
	runAnalysis("/data/Kinect_v2_Calibration.xml", "F:\\Kamera_Evaluation\\\Simulation\\Recordings_Path_Length_2");
