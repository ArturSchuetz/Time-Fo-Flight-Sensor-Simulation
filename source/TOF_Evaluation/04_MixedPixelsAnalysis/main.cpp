...
void runAnalysis(const std::string& calibrationFilePath, const std::string& recordingsFolderPath, unsigned int analyse_row)
{
	////////////////////////////////////////////////////////////////
	// Preperations
	////////////////////////////////////////////////////////////////
	...
	std::cout << "rgbToIrCameraViewMatrix" << rgbToIrCameraViewMatrix << std::endl;
	std::cout << "irToRgbCameraViewMatrix" << irToRgbCameraViewMatrix << std::endl;

	cv::Mat_<cv::Vec4f> ir_directionMatrix = bow::CameraCalibration::calculate_directionMatrix(ir_intrinisicCameraParameters, ir_intrinisicCameraParameters.image_width, ir_intrinisicCameraParameters.image_height, true);
	// ==============================================================
	...
	cv::Mat_<ushort>* mean_depth_maps = new cv::Mat_<ushort>[recordedFiles.size()];
	unsigned int currend_depth_map = 0;
	for (unsigned int dirIndex = 0; dirIndex < recordedFiles.size(); dirIndex++)
	{
		...
		if (recordedFiles[dirIndex].depthFiles.size() > 0)
		{
			...
			mean_depthMat.copyTo(mean_depth_maps[currend_depth_map++]);
		}
		...
		if (mean_depthMat.cols > 0 && mean_depthMat.rows > 0 && reference_depthMat.cols > 0 && reference_depthMat.rows > 0)
		{
			...
			for (unsigned int col = 0; col < coordinates.cols; col++)
			{
				cv::Vec3f depthVal = coordinates.at<cv::Vec3f>(analyse_row, col);
				cv::Vec3f depthRefVal = coordinatesRef.at<cv::Vec3f>(analyse_row, col);
				if (csv_file.is_open())
				{
	...
	std::ofstream csv_file;
	csv_file.open(recordingsFolderPath + "\\" + recordedFiles[0].folderName + "_cleaned.csv", std::ofstream::out | std::ofstream::trunc);

	std::ofstream depth_datFile;
	depth_datFile.open(recordingsFolderPath + "\\" + recordedFiles[0].folderName + "_depth_cleaned.dat", std::ofstream::out | std::ofstream::trunc);

	std::ofstream depth_datFile_foreground;
	depth_datFile_foreground.open(recordingsFolderPath + "\\" + recordedFiles[0].folderName + "_depth_cleaned_foreground.dat", std::ofstream::out | std::ofstream::trunc);

	cv::Mat_<cv::Vec3f> coordinates = bow::CameraCalibration::calculate_coordinates_from_depth(ir_directionMatrix, mean_depth_maps[1]);
	cv::Mat_<cv::Vec3f> coordinates_masked = bow::CameraCalibration::calculate_coordinates_from_depth(ir_directionMatrix, mean_depth_maps[2]);
	for (unsigned int col = 0; col < coordinates.cols; col++)
	{
		cv::Vec3f depthVal = coordinates.at<cv::Vec3f>(analyse_row, col);
		cv::Vec3f depthVal_ref = coordinates_masked.at<cv::Vec3f>(analyse_row, col);
		if (csv_file.is_open())
		{
			if (depthVal.val[2] > 0 && (coordinates_masked.at<cv::Vec3f>(analyse_row, col).val[2] > 1110 || coordinates_masked.at<cv::Vec3f>(analyse_row, col).val[2] < 800))
			{
				std::string value = std::to_string((double)depthVal.val[0]);
				std::replace(value.begin(), value.end(), '.', ',');
			...
			else if (depthVal_ref.val[2] > 0)
			{
				std::string value = std::to_string((double)depthVal_ref.val[0]);
				std::replace(value.begin(), value.end(), '.', ',');
				...
				csv_file << value
					...
					<< ";" << depthVal_ref.val[2]
					<< std::endl;
				depth_datFile_foreground << std::to_string(depthVal_ref.val[0]) << " " << std::to_string(depthVal_ref.val[2]) << std::endl;
			}
		}
	}
	csv_file.close();
	...
	delete[] mean_depth_maps;
}

int main()
{
	g_renderer.Start();
	//runAnalysis("/data/IFM_O3D303_Calibration.xml", "F:\\Kamera_Evaluation\\03D303\\Recordings_Mixed_Pixels", 180);
	runAnalysis("/data/Kinect_v2_Calibration.xml", "F:\\Kamera_Evaluation\\Kinect_v2\\Recordings_Mixed_Pixels", 194);
	//runAnalysis("/data/Xtion_2_Calibration.xml", "F:\\Kamera_Evaluation\\Xtion_2\\Recordings_Mixed_Pixels", 194);
	g_renderer.Stop();
	return 0;
}