...
namespace bow {
	...
	void ImageLoader_tga::ImportImage(const char* inputData, Image* outputImage)
	{
		...
		unsigned int m_sizeInBytes = outputImage->m_width * outputImage->m_height * (outputImage->m_numBitsPerPixel / 8);

		// allocate the image-buffer
		std::vector<unsigned char> raw_data(outputImage->m_sizeInBytes);
