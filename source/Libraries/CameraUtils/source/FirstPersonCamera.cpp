...
namespace bow
{
	...
	FirstPersonCamera::FirstPersonCamera(const bow::Vector3<double>& cameraPosition, const bow::Vector3<double>& lookAtPoint, const bow::Vector3<double>& worldUp, unsigned int width, unsigned int height)
		: bow::Camera(cameraPosition, lookAtPoint, worldUp, width, height),
	...
	{
		bow::Vector3<float> delta = m_Dir * m_Speed * (float)deltaTime;
		m_Position += delta;
	...
	void FirstPersonCamera::MoveRight(float deltaTime)
	{
		...
		bow::Vector3<float> vcRight(CrossP(m_Dir, vcUp));

		bow::Vector3<float> delta = vcRight * m_Speed * (float)deltaTime;
		m_Position += delta;
	...
	{
		bow::Vector3<float> delta = m_Up * m_Speed * (float)deltaTime;
		m_Position += delta;
