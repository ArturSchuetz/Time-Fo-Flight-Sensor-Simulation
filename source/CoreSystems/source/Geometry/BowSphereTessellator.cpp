...
namespace bow {
	...
	MeshAttribute SubdivisionSphereTessellator::ComputeFromIcosahedron(int numberOfSubdivisions, bool normalize)
	{
		...
		Vector3<float> v0 = Vector3<float>(-X, 0.0, Z); v0.Normalize();
		Vector3<float> v1 = Vector3<float>(X, 0.0, Z); v1.Normalize();
		Vector3<float> v2 = Vector3<float>(-X, 0.0, -Z); v2.Normalize();
		Vector3<float> v3 = Vector3<float>(X, 0.0, -Z); v3.Normalize();
		Vector3<float> v4 = Vector3<float>(0.0, Z, X); v4.Normalize();
		Vector3<float> v5 = Vector3<float>(0.0, Z, -X); v5.Normalize();
		Vector3<float> v6 = Vector3<float>(0.0, -Z, X); v6.Normalize();
		Vector3<float> v7 = Vector3<float>(0.0, -Z, -X); v7.Normalize();
		Vector3<float> v8 = Vector3<float>(Z, X, 0.0); v8.Normalize();
		Vector3<float> v9 = Vector3<float>(-Z, X, 0.0); v9.Normalize();
		Vector3<float> v10 = Vector3<float>(Z, -X, 0.0); v10.Normalize();
		Vector3<float> v11 = Vector3<float>(-Z, -X, 0.0); v11.Normalize();
	...
	Vector2<float> SubdivisionSphereTessellator::ComputeTextureCoordinate(Vector3<float> position)
	{
		return Vector2<float>(
			(float)((atan2((float)position.y, (float)position.x)) * (1.0 / (2.0 * M_PI)) + 0.5),
			(float)(asin((float)position.z) * (1.0 / M_PI) + 0.5)
			);
	}
}
